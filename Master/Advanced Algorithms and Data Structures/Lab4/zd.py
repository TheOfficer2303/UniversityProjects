    ...
    def _query(self, node: IntervalNode, qx1: float, qx2: float, qy1: float, qy2: float) -> list:
        ...
        if is_in_range(qx1, qx2, node.x_median):
          for interval in node.intervals:
            ...
            if is_in_window((ix1, iy), None, qx2, qy1, qy2) or is_in_window((ix2, iy), qx1, None, qy1, qy2):
              iv.add(interval)
          move = 'both'
          ...
          for interval in node.intervals:
            ...
            if is_in_window((ix2, iy), qx1, None, qy1, qy2):
              iv.add(interval)
          move = 'right'
          ...
          for interval in node.intervals:
            ...
            if is_in_window((ix1, iy), None, qx2, qy1, qy2):
              iv.add(interval)
          move = 'left'

        if move in ['both','left'] and node.left_child is not None:
            iv = iv.union(self._query(node.left_child, qx1, qx2, qy1, qy2))
        if move in ['both', 'right'] and node.right_child is not None:
            iv = iv.union(self._query(node.right_child, qx1, qx2, qy1, qy2))
