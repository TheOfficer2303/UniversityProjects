...
contract Auction {
    ...
    function finalize() public {
        require(getAuctionOutcome() == Outcome.SUCCESSFUL);

        if (judgeAddress != address(0)) {
            require(msg.sender == judgeAddress || msg.sender == getHighestBidder());
    ...
    function refund() public {
        require(getAuctionOutcome() == Outcome.NOT_SUCCESSFUL);
        require(getHighestBidder() != address(0));

        if (judgeAddress != address(0)) {
            require(msg.sender == judgeAddress || msg.sender == sellerAddress);
            payable(getHighestBidder()).transfer(address(this).balance);
        } else {
            payable(getHighestBidder()).transfer(address(this).balance);
        }
    }
