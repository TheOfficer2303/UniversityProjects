...
class ID3:
    ...
    def id3(self, D: DataSet, D_parent: DataSet, X, y, depth):
        ...
        for v in D.feature_values[x]:
            ...
            if depth < self.max_depth:
                t = self.id3(Dx_v, D, X, y, depth+1)
                if t is None:
                    t = Leaf(Dx_v.most_common_label)
            else:
                X.append(x)
                return
            subtrees.append((v, t))
    ...
    def fit(self, train_dataset: DataSet, max_depth):
        self.max_depth = max_depth
        self.decision_tree = self.id3(train_dataset, train_dataset, train_dataset.feature_names[:-1], train_dataset.feature_names[-1], depth=0)
    ...
    def confusion_matrix(self, predictions, test_dataset):
        possible_predictions = test_dataset.get_classes_values()
        matrix_len = test_dataset.get_number_of_classes()
