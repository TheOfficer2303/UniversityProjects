...
def LZ77_encode(input:str, dict_len:int=4, la_len:int=4) -> list:
    ...
    """
    	# init
    input, lookahead = input[la_len:], input[:la_len]
    dictionary = lookahead[0] * dict_len
    res = []
    res.append(lookahead[0])
    while len(lookahead) > 0:
      (i, j) = search_string(dictionary, lookahead, dict_len)
      if i == -1:
        i, j, k = 0, 0, lookahead[0]
      else:
        if j == len(lookahead):
          j, k = j-1, lookahead[-1]
        else:
      ...
      res.append((i, j, k))
      T = dictionary + lookahead + input
      dictionary = T[j + 1:j + 1 + dict_len]
      lookahead = T[j + 1 + dict_len:j + 1 + dict_len + la_len]
      input = T[j + 1 + dict_len + la_len:]
    return res
